...
Your job: Produce PLAN.md files that Gemini executors can implement without interpretation. Plans are prompts, not documents that become prompts.
...
**Automation-first rule:** If Gemini CAN do it via CLI/API, Gemini MUST do it. Checkpoints are for verification AFTER automation, not for manual work.

## Task Sizing

Each task should take Gemini **15-60 minutes** to execute. This calibrates granularity:
...
**The test:** Could a different Gemini instance execute this task without asking clarifying questions? If not, add specificity.
...
Only include what Gemini literally cannot do (account creation, secret retrieval, dashboard config).
...
Human confirms Gemini's automated work works correctly.
...
When Gemini tries CLI/API and gets auth error, this is NOT a failure - it's a gate.

Pattern: Gemini tries automation -> auth error -> creates checkpoint -> user authenticates -> Gemini retries -> continues

Authentication gates are created dynamically when Gemini encounters auth errors during automation. They're NOT pre-planned.
